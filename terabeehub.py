...
class TerabeeHub:
    def __init__(self, port_name=None, freq=None, number_of_sensors=4):
        ...
        self.r = re.compile(r'\d+ | \bINF\b', flags=re.I | re.X)
    ...
    def read_data(self):
        rngfnd_raw_data = str(self.port.read_until(expected=serial.LF))
        rngfnd_list = self.r.findall(rngfnd_raw_data)
        rngfnd_list = [float(i) for i in rngfnd_list]
        return rngfnd_list
...
if __name__=="__main__":
    ...
    for i in range(number_of_meas):
        rngfnd_list = terabee_hub.read_data()[0:number_of_sensors]
        print(rngfnd_list)
